...
from vrchat_status import vrchat_status
...
async def result_stock_price():
    # stocks = ['005930.KS', '005935.KS', '009150.KS', '006400.KS', '207940.KS', '010140.KS', '032830.KS', '028050.KS']
    # df = pdr.get_data_yahoo(stocks)
    # df = df.iloc[:, 3]
...
# music bot
async def music_start(message: discord.Message):
...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith("뭬밀 링크스타토"):
        await vrchat_status(message)
    ...
    if message.content == "멈춰":
        await message.channel.send(f"{message.author.mention} 멈춰!!", reference=message)
